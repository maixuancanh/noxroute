...
contract NoxBatchRouter {
    ...
    address public immutable user0;
    ...
    constructor(
        address configuredCoordinator,
        address[3] memory configuredUsers,
        ...
        if (configuredCoordinator == address(0)) revert InvalidCoordinator();
        ...
        for (uint256 i = 0; i < 3; i++) {
            address configuredUser = configuredUsers[i];
            if (configuredUser == address(0)) revert InvalidUser(configuredUser);
            for (uint256 j = 0; j < i; j++) {
                if (configuredUser == configuredUsers[j]) revert InvalidUser(configuredUser);
            }
        }

        coordinator = configuredCoordinator;
        user0 = configuredUsers[0];
        user1 = configuredUsers[1];
        user2 = configuredUsers[2];
    ...
    function submitIntent(
        ...
        uint8 userIndex = _validateIntent(
            epochId,
    ...
    function _validateIntent(
        ...
        bytes32 encryptedMinOutHandle,
        uint128 escrowCap
    ) private view returns (uint8 userIndex) {
        ...
        userIndex = _userIndex(msg.sender);
        if (amountHandles[epochId][userIndex] != bytes32(0)) revert IntentAlreadySubmitted();
    ...
    function _storeIntent(
        ...
        amountHandles[epochId][userIndex] = encryptedAmountHandle;
        minOutHandles[epochId][userIndex] = encryptedMinOutHandle;
        amountProofs[epochId][userIndex] = encryptedAmountProof;
        minOutProofs[epochId][userIndex] = encryptedMinOutProof;
        escrowCaps[epochId][userIndex] = escrowCap;
    ...
    function requestNetting(bytes32 epochId) external nonReentrant {
        ...
        if (epochState.intentCount != 3) revert IntentBatchIncomplete();
    ...
    function settle(bytes32 epochId, uint128 minAmountOut) external nonReentrant {
        ...
        if (minAmountOut == 0 || minAmountOut > epochState.totalOutput) {
            revert InvalidAllocation();
        }
        ...
        for (uint8 i = 0; i < 3; i++) {
            ...
            if (refund != 0) _safeTransfer(tokenIn, _userAt(i), refund);
            if (outputAmounts[epochId][i] != 0) {
                _safeTransfer(tokenOut, _userAt(i), outputAmounts[epochId][i]);
            }
        }
    ...
    function cancelEpoch(bytes32 epochId) external nonReentrant {
        ...
        for (uint8 i = 0; i < 3; i++) {
            ...
            if (escrow != 0) {
                ...
                _safeTransfer(tokenIn, _userAt(i), escrow);
            }
        }
    ...
    function userAt(uint256 index) external view returns (address) {
        if (index > 2) revert InvalidUser(address(0));
        return _userAt(uint8(index));
    }
    ...
    function _userAt(uint8 index) private view returns (address) {
        if (index == 0) return user0;
        if (index == 1) return user1;
        return user2;
    }

    function _userIndex(address account) private view returns (uint8) {
        if (account == user0) return 0;
        if (account == user1) return 1;
        if (account == user2) return 2;
        revert UnauthorizedUser();
    }

    function _evaluationTimedOut(Epoch storage epochState) private view returns (bool) {
        return
            block.timestamp >= epochState.requestedAt &&
            block.timestamp - epochState.requestedAt >= evaluationTimeout;
    }
